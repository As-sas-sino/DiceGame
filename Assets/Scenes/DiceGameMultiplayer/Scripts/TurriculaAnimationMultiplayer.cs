...
public class TurriculaAnimationMultiplayer : MonoBehaviour
{
    ...
    public DadoControllerMultiplayer dice;  // Riferimento al dado da lanciare
    ...
    private void ReleaseDice()
    {
        ...
        dice.transform.SetParent(null, true);
        dice.ForceWorldScale();
        Debug.Log("WorldScale after fix: " + transform.lossyScale);

        dice.LanciaDado();

        // nascondi dopo (non subito)
        StartCoroutine(HideAfterDelay());
    }

    private IEnumerator HideAfterDelay()
    {
        yield return new WaitForSeconds(1f); // identico AR
        HideBowl();
    }
